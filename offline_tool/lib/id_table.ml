...
let init_from_map id_map ~f =
  ...
  in
  let t = Array.create ~len None in
  Map.iteri id_map ~f:(fun ~key:id ~data:metadata ->
...
let find t id =
  ...
  if id' < 0 || id' >= Array.length t then None else Array.unsafe_get t id'
;;
...
let fold t ~init ~f =
  Array.foldi t ~init ~f:(fun id accum data ->
    match data with
...
let fold_right t ~init ~f =
  let rec loop i accum =
    let accum =
      ...
      | None -> accum
    in
    if i <= 0 then accum else loop (i - 1) accum
  in
  loop (Array.length t - 1) init
;;

let to_alist t = fold_right t ~init:[] ~f:(fun accum id item -> (id, item) :: accum)
