...
type t =
  ...
  ; epoch : Profiler_epoch.t (* Read only tables, but the cells are mutable records. *)
...
let create epoch id_map interests buffer =
  ...
  ; epoch
  ; group_state =
      Id_table.filter_map id_map ~f:(fun _id header_item ->
        match header_item with
        | Reader.Header.Item.Group _ -> Some { current_session = 0; session_at_count = 0 }
        | Single _ | Group_point _ -> None)
  ; point_state =
      Id_table.filter_map id_map ~f:(fun _id header_item ->
        match header_item with
        | Reader.Header.Item.Group_point _ ->
          Some { session = -1; time = Time_ns.epoch; value = -1; at_index = 0 }
        | Single _ | Group _ -> None)
...
let iter_events t ~f =
  Reader.iter_short_messages t.buffer t.epoch t.id_map ~f:(fun message ->
    ...
    in
    let group_state =
      lazy (Option.value_exn group_point_parent |> Id_table.find_exn t.group_state)
    in
