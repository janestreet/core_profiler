  ...
  | In_interval (rx, sx, ux, ivx), In_interval (ry, sy, uy, ivy) ->
    ...
    then c
      ...
      then c
        ...
        if c <> 0 then c else interval_compare ivx ivy))
...
let lookup_ids' t (name_map : Util.Name_map.t) =
  let lookup_raw r =
    let open Raw in
    match r with
    | Single name -> Single (Map.find_exn name_map.singles name)
...
let lookup_ids t name_map =
  try lookup_ids' t name_map with
...
let lookup_names t id_map =
  ...
  let lookup_raw r =
    ...
    | Group_point (group, point) -> Group_point (get_name group, get_name point)
...
let coerce_interval_units t id_map =
  ...
  | In_interval (raw, iv_subject, iv_units, interval) ->
    let desired =
      ...
      | Time_delta -> Profiler_units.Nanoseconds
    in
    let interval =
      Interval.Int.map interval ~f:(Util.coerce_units ~current:iv_units ~desired)
    in
...
let examples =
  ...
  ; In_interval
      (Single "some_probe", Value, Profiler_units.Words, Interval.Int.create 20_000 20_100)
  ; In_interval
...
let readme =
  lazy
    ([ `S
         "An interest is a string that specifies some subset of the core-profiler file \
          that you are interested in / wish to inspect.\n\n"
     ; `S "You can ask for an individual timer or probe by specifying its name:\n\n"
     ; `S "    "
     ; `E (All (Single "probe_or_timer_name"))
     ; `S "\n\n"
     ; `S "And inspect the values (no deltas) of a point of a probe group like so:\n\n"
     ; `S "    "
     ; `E (All (Group_point ("group1", "single_point_name")))
     ; `S "\n\n"
     ; `S
         "To get deltas from a group, you need to describe the path you're interested in.\n"
     ; `S (Lazy.force Path.readme)
     ; `S "\n\n"
     ; `S
         "You may also filter the data by demanding that the value, delta or time_delta \
          be in some interval. The syntax for filtering is as follows: \n\n"
     ; `S "    interest~subject[lower,upper]\n\n"
     ; `S
         "Where interest is a raw interest as described above, subject is one of 't', \
          'd' or 'v', and [lower, upper] specify the interval. The interval may be \
          specified using units appropriate to the probe filtered, e.g., '3ms' or \
          '1kw'.\n\n"
     ; `S "Some examples:\n\n"
     ; `S "    "
     ; `E (List.nth_exn examples 2)
     ; `S "\n"
     ; `S "    "
     ; `E (List.nth_exn examples 3)
     ; `S "\n"
     ; `S "    "
...
let list_arg = Command.Spec.(anon (sequence ("interest" %: arg_type)))
...
let default_interests id_map =
  ...
  let product_fold outer ~init ~get_inner ~f =
    List.fold outer ~init ~f:(fun acc b ->
      List.fold (get_inner b) ~init:acc ~f:(fun acc a -> f acc a b))
  in
  let add_group_default_interests accum group_id =
    let group_points = get_points_sources group_id in
    let some_defaults_specified =
      List.exists group_points ~f:(fun (_id, sources) -> not (List.is_empty sources))
    in
    ...
    then
      ...
      product_fold
        ...
        ~f:(fun acc (a, _) (b, _) ->
          ...
          All (Group_path (group_id, path)) :: acc)
  in
  Id_table.fold id_map ~init:[] ~f:(fun acc id header_item ->
