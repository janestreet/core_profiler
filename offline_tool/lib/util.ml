...
module Name_map = struct
  type group =
    { id : Probe_id.t
    ; children : Probe_id.t String.Map.t
    }

  type t =
    { singles : Probe_id.t String.Map.t
    ; groups : group String.Map.t
    }

  let of_id_map (id_map : Reader.Header.t) =
    let warn_if_duplicate what map key data =
      match Map.find map key with
      | Some _ -> printf !"Duplicate %s name %s; keeping %{Probe_id}" what key data
      | None -> ()
    in
    let collect_children group_id group_name =
      let warning_what = sprintf "group %s point" group_name in
      Id_table.fold id_map ~init:String.Map.empty ~f:(fun t id item ->
        match item with
        | Single _ | Group _ -> t
...
let int_units_of_string str =
  ...
  let getend n = if len >= n then str.[len - n] else ' ' in
  ...
  Float.iround_nearest_exn (number *. multiplier), units
;;
...
let coerce_units value ~current ~desired =
  ...
  | _ ->
    failwithf
      !"Don't know how to convert %{Profiler_units} to %{Profiler_units}"
      current
      desired
      ()
;;
