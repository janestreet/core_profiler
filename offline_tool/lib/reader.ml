...
module Header = struct
  ...
  let create_table
    ...
    Id_table.filter_map t ~f:(fun id header_item ->
      ...
      in
      if spec_ok && item_type_ok then Some empty else None)
  ;;
end
...
let consume_header buffer =
  ...
  let add_exn map id data =
    ...
    | Some existing ->
      failwithf
        !"Duplicate Id %{Probe_id} (%s, %s)"
        id
        (Header.Item.name existing)
        (Header.Item.name data)
        ()
    | None -> Map.set map ~key:id ~data
  in
  let add_point_to_group map ~group_id ~point_id =
    ...
    | Some (Header.Item.Single _ | Group_point _) | None ->
  ...
  let check_sources_parent map ~point_id ~group_id sources =
    List.iter sources ~f:(fun source_id ->
      ...
      | Some (Header.Item.Group_point pp) ->
        if pp.parent <> group_id
        then
          failwithf
            !"Point %{Probe_id} of group %{Probe_id} references source point %{Probe_id} \
              not belonging to the same group"
            point_id
            group_id
            source_id
            ()
      | Some (Header.Item.Single _ | Header.Item.Group _) | None ->
        failwithf
          !"Point %{Probe_id} of group %{Probe_id} references source point %{Probe_id} \
            that is not a group point"
          point_id
          group_id
          source_id
          ())
  ...
  let rec scan epoch map =
    ...
    | HP_MsgT.New_group ->
      HP.New_group.(
        ...
        let group_spec =
          { name = get_name message; Header.Item.points_spec; children = [] }
        in
        let map = add_exn map (get_id message) (Header.Item.Group group_spec) in
        scan epoch map)
    | HP_MsgT.New_group_point ->
      HP.New_group_point.(
        let sources =
        ...
        let metadata =
          Header.Item.Group_point { name = get_name message; parent = group_id; sources }
        in
    ...
    | HP_MsgT.Epoch ->
      HP.Epoch.(
        (match epoch with
         | Some epoch ->
           failwithf
             !"Header contained two epochs: %{Profiler_epoch}, %{Profiler_epoch}"
             epoch
             (get_epoch message)
             ()
         | None -> scan (Some (get_epoch message)) map))
    | HP_MsgT.End_of_header -> epoch, map
    | HP_MsgT.Need_more_data -> failwith "Invalid header (truncated)"
...
let%test_unit "read_header" =
  ...
  protect ~finally:Buffer.Unsafe_internals.reset ~f:(fun () ->
    ...
    Writer.write_new_group_point ~id:(to_id 4) ~group_id:(to_id 1) "group1point4" [||];
    Writer.write_new_group_point ~id:(to_id 5) ~group_id:(to_id 2) "group2point5" [||];
    ...
    let expect =
      [ ( to_id 0
        , HI.Single { name = "probe0"; spec = Probe_type.Probe Profiler_units.Words } )
      ; ( to_id 1
        , HI.Group
            { name = "group1"; points_spec = Probe_type.Timer; children = [ to_id 4 ] } )
      ; ( to_id 2
        , HI.Group
            { name = "group2"
      ...
      ; ( to_id 6
        , HI.Group_point
            { name = "group2point6"; parent = to_id 2; sources = [ to_id 5 ] } )
      ; ( to_id 7
        , HI.Group_point
...
let consume_short_message buffer epoch header =
  ...
  if remaining = 0
  then failwith "Invalid short message: empty buffer"
  else if remaining < 8
  then failwith "Invalid short message: truncated"
  else (
    ...
    | HI.Group _ -> read_group_reset ()
    | HI.Single { spec; _ } -> read_by_spec spec
...
let%test_unit "consume_short_message" =
  ...
  protect ~finally:Buffer.Unsafe_internals.reset ~f:(fun () ->
    ...
    Writer.write_new_group_point ~group_id:(to_id 3) ~id:(to_id 4) "timer-group-point" [||];
    Writer.write_new_group (to_id 5) "probe-group" (Probe_type.Probe Profiler_units.Int);
    Writer.write_new_group_point ~group_id:(to_id 5) ~id:(to_id 6) "probe-group-point" [||];
    Writer.Unsafe_internals.write_end_of_header ();
