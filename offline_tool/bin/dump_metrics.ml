...
let rec name id_map id =
  ...
  | Single { name = n; _ } -> n
  | Group { name = n; _ } -> n
  | Group_point { name = n; parent; sources = _ } -> name id_map parent ^ "." ^ n
;;

let display_message (m : Reader.Short_message.t) id_map =
  match m with
  | Timer (id, time) -> printf !"%{Time_ns} %-20s mark\n" time (name id_map id)
  | Probe (id, time, value) -> printf !"%{Time_ns} %-20s %i\n" time (name id_map id) value
  | Group_reset (id, time) ->
...
let command =
  Command.basic_spec
    ...
    Command.Spec.(
      ...
      empty +> anon ("filename" %: iobuf_file_arg_type))
    main
;;
