...
let iobuf_file_flag () =
  Command.Spec.(
    ...
    |> flag
         "-filename"
         ~doc:(sprintf "%s core_profiler file to open" Protocol.default_output_filename))
;;

let lookup_coerce_interests id_map interests =
  let name_map = Name_map.of_id_map id_map in
  List.map interests ~f:(fun interest ->
    Interest.lookup_ids interest name_map |> Fn.flip Interest.coerce_interval_units id_map)
;;

let or_default_interests id_map interests =
  if List.is_empty interests then Interest.default_interests id_map else interests
;;
...
module Log_command = struct
  ...
  let context_of_string x =
    ...
    | value, Profiler_units.Int when value > 0 -> Lines value
  ...
  let max_name_length id_map filter =
    List.fold (Filter.raw_interests filter) ~init:0 ~f:(fun max interest ->
  ...
  let display_event id_map event ansi_attrs name_length =
    ...
    let units ri =
      ...
      | Single id | Group_point (id, _) | Group_path (id, _) ->
  ...
  let display_near id_map filter events near_filter near_context =
    ...
    let ( (matching_event : Time_ns.t -> bool -> unit -> unit)
      ...
      | Timespan near_tspan ->
        ...
        let matching_event time _mwbd () = state := time in
        let next_event time () =
    ...
    Event_generator.iter_events events ~f:(fun event ->
      ...
      if matches_near
      then (
        if not (Deque.is_empty dequeue)
        then (
          if !printed_once then print_string "---\n";
          Deque.iter' dequeue `back_to_front ~f:(fun m ->
            display_event id_map m [] name_length);
          Deque.clear dequeue);
  ...
  let main interests buffer nears near_context =
    ...
    let interests =
      lookup_coerce_interests id_map interests |> or_default_interests id_map
    in
    ...
    then (
      ...
      let raw_interests =
        Filter.raw_interests filter @ Filter.raw_interests near_filter
      in
      let events = Event_generator.create epoch id_map raw_interests buffer in
  ...
  let readme () =
    String_extended.word_wrap
      ~soft_limit:75
      "The log may be filtered by specifying some INTERESTs. It may be further filtered \
       by asking only for entries near lines that match -near INTERESTs. The amount of \
       context shown is controlled by -context and may be specified as a number of \
       messages (e.g., -context 10, the default) or time (e.g., -context 10ns)\n\n\
  ...
  let command =
    Command.basic_spec
      ~summary:
        "Show the (full, or filtered) stream of events (i.e., the log, having calculated \
         paths deltas)."
      ~readme
      Command.Spec.(
        empty
        ...
        +> flag
             "-near"
             (listed Interest.arg_type)
             ~doc:"INTEREST Restrict to messages near where this condition is true"
        +> flag
             "-context"
             (optional_with_default (Lines 10) context_arg_type)
...
module List_command = struct
  ...
  let main buffer prune () =
    let epoch, id_map = Reader.consume_header buffer in
    let id_map = if prune then prune_unused buffer epoch id_map else id_map in
    Id_table.fold_right id_map ~init:[] ~f:(fun accum id data ->
      match data with
      | Single { name; spec } -> (id, name, Some spec) :: accum
    ...
    |> Ascii_table.(
         output
           ...
           [ Column.create ~align:Left "id" (fun (id, _, _) -> Probe_id.to_string id)
           ; Column.create ~align:Left "name" snd3
           ; Column.create ~align:Left "spec" (fun (_, _, spec) ->
               Option.map spec ~f:Probe_type.to_string |> Option.value ~default:"")
           ])
  ;;
...
module Summary_command = struct
  ...
  module Stats = struct
    ...
    let create selections =
      { rstats =
          (if Selections.need_rstats selections then Some (Fstats.create ()) else None)
      ; order_stats =
    ...
    let update_in_place t v =
      t.is_empty <- false;
      Option.iter t.rstats ~f:(fun rstats -> Fstats.update_in_place rstats (float v));
    ...
    let percentile t x = Reservoir_sampling.percentile (Option.value_exn t.order_stats) x
  end
  ...
  let table_columns id_map (selections : Selections.t) =
    let add want name func acc =
      if want then Ascii_table.(Column.create ~align:Right name func) :: acc else acc
    in
    let percentiles =
      List.map selections.percentiles ~f:(fun p ->
        let name =
  ...
  let singles_and_points_rows id_map stats_table =
    ...
    Hashtbl.fold stats_table ~init:[] ~f:(fun ~key:interest ~data:stats accum ->
  ...
  let timer_paths_rows tbl =
    Hashtbl.fold tbl ~init:[] ~f:(fun ~key:interest ~data:stats accum ->
  ...
  let main interests buffer selections =
    let selections =
      if Selections.is_empty selections then Selections.default else selections
    in
    let epoch, id_map = Reader.consume_header buffer in
    let interests =
      lookup_coerce_interests id_map interests |> or_default_interests id_map
    ...
    List.iter interests ~f:(fun interest ->
      ...
      let res =
        ...
        | true, false -> Hashtbl.add probes ~key:interest ~data:(new_stats ())
        | false, true -> Hashtbl.add timer_paths ~key:interest ~data:(new_stats ())
        | true, true ->
          Hashtbl.add
            probe_paths
            ~key:interest
    ...
    Filter.iter_events_interests filter events ~f:(fun event interest ->
      ...
      | Probe_path { time_delta; delta; _ } ->
        let { time_delta_stats; delta_stats } = Hashtbl.find_exn probe_paths interest in
  ...
  let readme () =
    String_extended.word_wrap
      ...
      "If no statistics (sum, mean, ...) are requested, a default selection will be \
       shown. You can ask for multiple percentiles by passing the -percentile flag \
       several times.\n\n\
       The data may be filtered by specifying some INTERESTs; see the  help-interests  \
       subcommand."
  ;;
...
module Plot_command = struct
  ...
  let variable_arg = Command.Spec.(anon ("variable" %: variable_argtype))
  ...
  let plot_max_bins =
    Command.Spec.(
      flag "-bins" (optional_with_default 20 int) ~doc:"NUM Num of bins to plot.")
  ;;

  let plot_log_scale =
    Command.Spec.(
      flag "-log-scale" no_arg ~doc:" Show relative densities on a log10 scale.")
  ;;
  ...
  let get_range_reservoir filter events variable =
    ...
    Filter.iter_events filter events ~f:(fun event _interests ->
      let var =
        ...
        | Time, event ->
          Event_generator.event_time event |> Time_ns.to_int_ns_since_epoch |> Option.some
        | Value, (Probe (_, _, value) | Probe_path { value; _ }) -> Some value
        | Delta, Probe_path { delta; _ } -> Some delta
        ...
        | Time_delta, (Timer _ | Probe _) -> None
      in
  ...
  let density_bars ~max_num_bins ~log_scale (min, max) reservoir_samples =
    ...
    let num_bins =
      Int.min num_samples tot_width_p1 |> Int.max 1 |> Int.min max_num_bins
    in
    ...
    Array.mapi bins ~f:(fun i size ->
      ...
      , if log_scale
        then (
          ...
          if x < 1. then 0. else Float.log10 x)
  ...
  let label_density_bars bars formatter =
    let bars =
      List.map bars ~f:(fun ((low, high), size) -> (formatter low, formatter high), size)
    in
    let width_low, width_high =
      List.fold bars ~init:(0, 0) ~f:(fun (low_max, high_max) ((low, high), _) ->
  ...
  let main interest variable plot_perc max_num_bins log_scale buffer () =
    ...
    let bars =
      ...
      | Density ->
        let reservoir_samples = Reservoir_sampling.distribution reservoir in
    ...
    in
    bars |> Text_graph.render ~narrow:true |> print_endline
  ;;

  let readme () =
    String_extended.word_wrap
      ~soft_limit:75
      "Specify a single interest and the variable you would like plotted. The VARIABLE \
       must be one of time, value, delta or time_delta.\n\n\
       By default, a density plot is produced, with the 'variable' (time, value, ...) as \
       the independent variable, and frequency-of-occurrence as the dependent. \
