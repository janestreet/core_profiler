(** This interface file defines the [Profiler_intf] interface, which has three
    ...
    (2) By opening [Core_profiler.Std_online] you get an implementation where the
        profiling stats like mean, stddev etc are maintained online, i.e. in-process with
        the process that is being profiled. Running the online profiler causes the program
        to print a table of stats periodically to stdout as your program runs.

    (3) By opening [Core_profiler.Std_offline] you get the implementation where the
        profiling data is held in-memory in a buffer and is written out to a file at
        process exit and the stats can be analyzed offline. For most programs this is the
        preferred approach to profiling.
    ...
    (1) One can collect time-stamped metrics at various points in your programs using
        [Probe]s. The metrics are integers that represent some application specific
        quantity, for example the length of some list, the number of allocated words on
        the heap etc.
    ...
    (2) Alternately, one can use [Timer]s to collect time stamps without recording any
        ...
        only wants to measure *when something happened* and there is no associated
        quantity to measure. [Timer]s are strictly less general than [Probe]s, but are
...
module type Probe = sig
  ...
  (** A [Group] provides a way of grouping multiple probes (or timers). Once grouped, one
      ...
      [Core_profiler] supports a path query syntax where one can ask for stats
      about *paths* in the time series of data. The idea of a group is best explained by
      an example, which you can find in the tutorial. *)
  module Group : sig
    type t
    ...
    (** [sources] should be a list of probes, specifying the edges that we care about.
        ...
        profiler_tool. If no sources are specified, the offline tool will fall back to the
        default of "all two-probe direct paths" of a group. *)
...
module type Profiler_intf = sig
  module Profiler : sig
    ...
    (** [configure] lets one set various profiler parameters programmatically.
        ...
        You need to call this before any [Timer] or [Probe] has been created. If you set
        [don't_require_core_profiler_env] after a [Timer] or [Probe] has been created,
        then it will raise an exception if the value you are trying to set disagrees with
        that which was read from the environment.
        ...
        The environment variable [CORE_PROFILER] can be used to configure the app. Also
        see [core_profiler_env_help_string] below. *)
    val configure
    ...
    (** There are several slow operations that may happen occasionally when calling
        ...
        checks if they will be necessary soon, and does them in advance. If possible, call
        this (fairly regularly) from a time-insensitive point in code (or at least,
    ...
    (** In the online profiler, [dump_stats] prints a table of stats -- this is the same
        ...
        In the offline profiler, [dump_stats] writes out all the collected stats so far.
        This normally happens [at_exit] and this function lets the programmer dump the
        stats earlier. *)
  ...
  module Delta_probe : sig
    ...
    (** To measure changes in a value, one can call [start] followed by a call [stop]
        after some time. The call to [stop] will record the delta. Calls to [start]/[stop]
        must be interleaved for each [t].
        ...
        For each [t], there are two valid sequences of calls. The first is calling [start]
        then [stop]. The second is calling [start] then [pause] an arbitrary number of
        times, and ending with [record]. *)
    ...
    (** These are non-stateful and can be used in Async, wherein multiple jobs might call
        ...
        following. This function cannot be provided as part of the [Core_profiler] library
        because we'd like the library to be usable in [Async] and hence now depend on it.
