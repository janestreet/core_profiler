...
let add_slow_task kind f = slow_tasks := (kind, f) :: !slow_tasks

let () =
  add_slow_task Any_profiler (fun () ->
    Time_stamp_counter.Calibrator.calibrate (force Time_stamp_counter.calibrator))
;;
...
let maybe_do_slow_tasks' kind now reluctance =
  ...
  then (
    List.iter !slow_tasks ~f:(fun (orig_kind, g) ->
      if orig_kind = kind || orig_kind = Any_profiler then g ());
    last_slow_tasks := now)
;;
