...
let[@inline] of_iobuf_local (local_ buf) ~trusted:_ = exclave_ Iobuf.no_seek__local buf
...
module New_single = struct
  ...
  let set_name_zero_local buf f a =
    let lo = Iobuf.Lo_bound.window buf in
    let hi = Iobuf.Hi_bound.window buf in
    Iobuf.advance buf 5;
    Iobuf.resize buf ~len:64;
  ...
  let to_sub_iobuf_local t = exclave_
    Iobuf.sub_shared__local t ~len:(get_message_length t + 0)
  ;;
...
module New_group_point = struct
  ...
  let[@inline] get_sources_source_id buf ~count ~index =
  ...
  let[@inline] set_group_id buf field =
    let pos = 0 in
    let int_to_pack = Probe_id.to_int_exn field in
    Iobuf.Poke.uint16_le_trunc buf ~pos:(pos + 2) int_to_pack
  ;;
  ...
  let[@inline] set_sources_source_id buf ~count ~index field =
    if index < 0 || index >= count then invalid_arg "index out of bounds";
    let pos = 0 in
    let pos_of_source_id_using_count_and_index = pos + 72 + (count * 0) + (index * 2) in
    let int_to_pack = Probe_id.to_int_exn field in
    ...
    ; sources_grp =
        (let sources_count = get_sources_count buf in
         Array.init sources_count ~f:(fun i ->
           { Unpacked.source_id = get_sources_source_id buf ~count:sources_count ~index:i
           }))
    }
  ;;
...
module Epoch = struct
  ...
  let[@inline] set_epoch buf field =
    let pos = 0 in
...
let[@inline] num_bytes_in_message buf =
  let pos = 0 in
