...
let online_profiler_is_used () = we_are_using_online_profiler := true
...
let online_print () =
  if (not !internal_disable_print) && !we_are_using_online_profiler
  then (
    Core.printf !"%{Time}\n%!" (Time.now ());
    let table =
      Map.fold_right !all_rows ~init:[] ~f:(fun ~key:name ~data:(row_fn, units) acc ->
        let row = row_fn () in
        if row_is_empty row then acc else (name, units, row) :: acc)
    in
    if not (List.is_empty table)
    then (
      Ascii_table.output ~oc:Out_channel.stdout ~limit_width_to:150 columns table;
...
let maybe_print =
  ...
  fun () ->
    ...
    then (
      ...
      let diff = Time_ns.diff now !last_print |> Time_ns.Span.to_int_sec in
      if debug then Core.printf "print_interval = %d, diff = %d\n" !print_interval diff;
...
let () = at_exit online_print

let add_row : string -> (unit -> row) -> Profiler_units.t -> unit =
  add_print_to_slow_tasks ();
  fun name fn units -> all_rows := Map.set !all_rows ~key:name ~data:(fn, units)
;;

let safe_to_delay () = Common.maybe_do_slow_tasks Common.Online_profiler ~reluctance:1
...
module Profiler = struct
  ...
  let configure
    ...
    Option.iter online_print_by_default ~f:(fun bool -> print_enabled := bool);
    Option.iter online_print_time_interval_secs ~f:(fun secs -> print_interval := secs)
  ;;
end
...
module Timer = struct
  ...
  module Raw_group = struct
    type t =
      { name : string (* [session] is initialised to 0 *)
      ; mutable session : int
      }
  ...
  let create ~name = Single (Single.create name ())
...
module Probe = struct
  ...
  module Raw_group = struct
    type t =
      { name : string
      ; units : Profiler_units.t (* [session] is initialised to 0 *)
      ; mutable session : int
      }
  ...
  module Group_probe = struct
    type t =
      { name : string
      ; group : Raw_group.t
      ; sources : (t * Fstats.t) array (* See [Timer.Group.Probe.t] *)
      ; mutable session : int
    ...
    let record t value =
      ...
      for i = 0 to Array.length t.sources - 1 do
        let src, stats = t.sources.(i) in
        if src.session = gsession
        then Fstats.update_in_place stats (float (value - src.last_value))
      done;
      t.last_value <- value;
      t.session <- gsession;
  ...
  let create ~name ~units = Single (Single.create name units)
...
module Delta_timer = struct
  ...
  let create ~name =
    let t = { name; stats = Fstats.create (); state = Time_ns.epoch; accum = 0 } in
    online_profiler_is_used ();
  ...
  let update_in_place t d = Fstats.update_in_place t.stats (float d)
  ...
  let start t = t.state <- Common.now Common.Online_profiler ~reluctance:4 ()
...
module Delta_probe = struct
  ...
  let create ~name ~units =
    let t = { name; stats = Fstats.create (); state = 0; accum = 0 } in
    online_profiler_is_used ();
  ...
  let start t value = t.state <- value
  let pause t value = t.accum <- t.accum + value - t.state
