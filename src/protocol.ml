...
let set_current_output_filename v = current_output_filename := v
...
module Short_header : sig
  ...
  let pack_exn epoch id time =
    ...
    let time = Profiler_epoch.diff epoch time |> Time_ns.Span.to_int_ns in
    if time < 0 || time > max_time_diff_int || id < 0 || id > max_id
    then raise (Invalid_argument "parameter out of range")
    else time lor (id lsl time_bits)
  ;;
  ...
  let unpack_id header = header lsr time_bits |> Probe_id.of_int_exn
...
module Buffer : sig
  ...
  let allocate_new_chunk len =
    ...
    if not (Iobuf.is_empty current_chunk)
    then (
    ...
    protect
      ~f:(fun () ->
        ...
        | [ second; first ] ->
          ...
          [%test_eq: string] (Iobuf.to_string second) "the second chunk\n"
        | _ -> assert false)
  ...
  let%test_unit "get_header_chunk" =
    protect
      ~f:(fun () ->
        ...
        let contents = get_header_chunk () |> Iobuf.to_string in
  ...
  let%test_unit "get_chunks" =
    protect
      ~f:(fun () ->
        ...
        allocate_new_chunk 1000;
        Iobuf.Fill.stringo current_chunk "the second chunk";
        let contents = get_chunks () |> List.map ~f:(fun buf -> Iobuf.to_string buf) in
...
module Writer = struct
  ...
  let write_new_single id name spec =
    let header_chunk = Lazy.force Buffer.header_chunk in
    let written = Header_protocol.New_single.write ~id ~spec ~name header_chunk in
    Iobuf.advance header_chunk written
  ;;

  let write_new_group id name spec =
    let header_chunk = Lazy.force Buffer.header_chunk in
    let written = Header_protocol.New_group.write ~id ~spec ~name header_chunk in
    Iobuf.advance header_chunk written
  ;;
  ...
  let write_probe_at id time value =
    ...
    Iobuf.Unsafe.Poke.int64_le
      ...
      ~pos:0
      (Short_header.pack_unsafe epoch id time);
    Iobuf.Unsafe.Poke.int64_le current_chunk ~pos:8 value;
  ...
  let write_to_fd fd header_chunk chunks =
    List.iter (header_chunk :: chunks) ~f:(fun chunk ->
      Iobuf.protect_window_bounds_and_buffer chunk ~f:(fun chunk ->
  ...
  let write_to_file name_ref header_chunk chunks =
    ...
    (match Sys_unix.file_exists name with
     ...
     | `No | `Unknown -> ());
    Unix.with_file name ~mode:[ Unix.O_CREAT; Unix.O_WRONLY; Unix.O_TRUNC ] ~f:(fun fd ->
      write_to_fd fd header_chunk chunks)
  ;;
  ...
  let () = at_exit (fun () -> Option.iter !at_exit_handler ~f:dump_stats_internal)
